...
// argo-cd v3.5.3 vendors gitops-engine into its own repo and only wires it up via a local
// replace in its own go.mod, which this module (as a consumer) doesn't inherit. Point it at a
// resolvable commit from the same v3.5.3 tag instead.
replace github.com/argoproj/argo-cd/gitops-engine => github.com/argoproj/argo-cd/gitops-engine v0.0.0-20260914071520-c9c369efcc5b

// argo-cd v3.5.3 depends on k8s.io/kubernetes, whose own go.mod replaces its staging modules
// (k8s.io/kubelet, k8s.io/api, etc.) with local filesystem paths — also not inherited by
// consumers. Mirrored here from argo-cd v3.5.3's own go.mod replace block so those staging
// modules resolve to their real v0.36.1 releases instead of the unresolvable v0.0.0 placeholder.
replace (
