...
**Update, after this workstream and workstream 7 both landed**: octocov no longer runs inside the `go` job as described below — it moved to the `report` job (workstream 8) once `go` and `e2e` both produced a Go coverage profile and it made sense to centralize both test-result *and* coverage reporting in one place, instead of `go` reporting only its own slice of coverage. `go` and `e2e` now each upload their raw `coverage.out`/`coverage-e2e.out` as a `*-coverage-profile` artifact; `report` downloads both and points `.octocov.yml`'s `coverage.paths:` at both, letting octocov merge them (it folds overlapping coverage rather than double-counting, confirmed via `octocov dump report` against each input alone before landing this — merging does shift to line-based rather than statement-based coverage, a real but expected side effect of octocov's own merge design, not a bug). The design below (single-job, `go`-only coverage) is what actually shipped first, before that follow-up; kept for history.
