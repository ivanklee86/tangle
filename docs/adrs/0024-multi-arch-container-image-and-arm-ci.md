...
- Bad, because matrixing renames status checks: `go` becomes `go (amd64)`/`go (arm64)` and `e2e` becomes `e2e (amd64)`/`e2e (arm64)`. Branch-protection rules naming `go` or `e2e` silently stop being satisfiable by anything, which fails closed (PRs block) rather than open — but it blocks every PR until the rules are updated. This needs doing in the same change window, and it is the one step that cannot be done from this repository's files. Both jobs carry an explicit `name:` to get those names: GitHub's default is derived from *every* matrix dimension (`go (amd64, ubuntu-latest)`), which would embed the runner label in a name branch protection depends on, so a future `ubuntu-latest` bump would silently rename a required check a second time. Observed on the first CI run of this change, not predicted.
