...
`ArgoCDClient` holds a `connection{applications, closer, generation}` behind a mutex. All three RPC methods funnel through one generic helper that runs the call, and — if the failure says the connection itself is gone and the request context is still live — redials once and runs it again. `reconnect` takes the connection the caller actually used and redials only if that is still the current one, so N concurrent failures from a pool fan-out collapse into a single dial — provided that dial succeeds. A failed redial leaves no current connection, so the callers behind it each dial for themselves; being locked out of recovery because one redial failed would be worse than a few redundant dials, and bounding those is the deferred rate limit's job, below. Old connections are closed, which stops the proxy server and unlinks its socket. The client gains a `Close()`, surfaced on `IArgoCDClient` and `IArgoCDWrapper` and called from the graceful-shutdown block in `internal/tangle/server.go`.
...
- Bad, because the retry can duplicate a hard refresh. `GetManifests` issues `Get` with `Refresh: "hard"` first, and ArgoCD answers that by patching the application's refresh annotation and forcing an uncached `queryRepoServer` comparison — real work on the instance the pond pools exist to protect. Only a connection lost while ArgoCD was already working (`Canceled`) can duplicate it; in the `Unavailable` case the RPC never left this process. Repeating it is safe in the sense that the second refresh supersedes the first, but it is not free, and calling it "idempotent" (as an earlier draft of the plan did) understated that. We accept it: excluding that call from the retry would fail the diffs page in exactly the situation reconnecting was meant to rescue.
